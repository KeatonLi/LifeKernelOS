...
> 替代说明（2026-09-05）：MVP 已收敛为“首页 → 工作台 → 主线 → 任务 → 手动确认进度”。精力、可用时间、`DailyState`、候选筛选与 `selectedActionId` 不再属于 MVP；直接维护主线任务与进度由 `SPEC-0001` 承接。本文件保留为历史规格，不进入实现。
> 后续说明（2026-09-06）：`SPEC-0010` 在“多个长期目标 + 一个当前行动”的新模型下重新采用时间、精力和可解释候选，但不恢复本规格；新行为以 `SPEC-0010` 为准。
