...
- 同主线新增、编辑 To-do 后，不刷新页面即可读取最新列表及内容。
- 主线、当前行动及列表加载完成后统一更新；过期请求不能覆盖新的主线。
- 来自画像的 `#goal-<id>` 链接打开指定主线；无效来源回退到可访问主线。
- 读取失败显示错误并允许重试，不把失败伪装成空数据。
- 自动化覆盖见 `apps/web/src/reliability.test.tsx`；桌面与移动端人工验收仍待完成，状态保持 `Implemented`。
