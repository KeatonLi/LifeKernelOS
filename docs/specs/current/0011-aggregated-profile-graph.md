...
既有行为的修复，不扩展产品范围。前端回归入口：`npm run test:web`。

- 图谱根据主线及知识数量分配位置，不再循环复用五个坐标。
- 布局自动化覆盖 0、1、5、6、12 条主线以及每条主线的密集知识节点。
- 主线来源链接能够打开对应的主线，而不是被当前行动所属主线覆盖。
- 首次加载失败停止加载并显示重试入口；重试成功恢复真实数据或空状态。
- 自动化覆盖见 `apps/web/src/reliability.test.tsx`；真实浏览器布局与移动端验收仍待完成，状态保持 `Implemented`。
